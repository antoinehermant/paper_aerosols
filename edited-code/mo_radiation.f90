...
MODULE mo_radiation
  ...
  USE mo_control,         ONLY: lcouple, lmidatm, lprp
  USE mo_prp,             ONLY: prp
  ...
  USE mo_radiation_parameters, ONLY: ldiur, lradforcing, ldiag_aeropt,            &
                                     ...
                                     ighg, fco2, nmonth, iaero, iaero_forcing,    &
                                     ...
                                     decl_sun_cur,solar_parameters,solc_multiplier
  USE mo_radiation_forcing,ONLY: prepare_forcing

  USE mo_rrtm_params,   ONLY : nbndsw
  USE mo_srtm_setup,    ONLY : ssi_default, ssi_preind, ssi_amip, ssi_cmip6_picontrol, &
                             & ssi_cmip6_1950, ssi_RCEdiurnOn, ssi_RCEdiurnOff
...
CONTAINS
  ...
  SUBROUTINE pre_radiation
    ...
    IF (lrad .AND. l_trigrad) THEN
      ...
      CASE (7)
        solcm = SUM(ssi_cmip6_1950)
        ssi_factor = ssi_cmip6_1950
      CASE default
        WRITE (message_text, '(a,i2,a)') &
             'isolrad = ', isolrad, ' in radctl namelist is not supported'
        CALL message('pre_radiation', message_text)
  ...
  SUBROUTINE radiation(                                                       &
       ...
       & ,sw_net            ,ozone           ,swup_clrsfc      ,swdn_clrsfc   )
    ...
    REAL(wp), INTENT(OUT) ::      &
         ...
         swup_clrsfc(kbdim),      & !< SW upward flux at the surface (clear-sky)
         swdn_clrsfc(kbdim)         !< SW downward flux at the surface (clear-sky)

    REAL(wp), INTENT(INOUT) ::    & ! Avoid leaving kproma+1:kbdim undefined
         ozone(kbdim,klev)          !< Ozone 

    INTEGER  :: jk, jl, idx(kbdim), number_rad_call, i_rad_call
    INTEGER, DIMENSION(2) :: iaero_call=(/ 0, 10 /)  ! 10 for all anthropogenic aerosol sources
    ...
    DO i_rad_call = 1,number_rad_call
      iaero_call(1) = iaero
      IF (i_rad_call < number_rad_call) iaero_call(1) = iaero_forcing
      ! IMPORTANT REMARK: If radiation is called repeatedly, the LAST
      ! call is with all aerosols used for model time integration.
      ! IF aerosol optical properties shall be diagnosed, no problem occurs
      ! Since the writing of aerosol optical properties is at the end of a time step
      ! at which the full aerosols are written inside psrad_interface into the stream.
      
      !
      ! Call to PRP diagnostics
      !
      IF (lprp) CALL prp( &
    ...
    ! --- Clear sky fluxes
    ! sfc up and dn
    swup_clrsfc(1:kproma) = flx_upsw_clr(1:kproma, klevp1) * flux_factor(1:kproma)
    swdn_clrsfc(1:kproma) = flx_dnsw_clr(1:kproma, klevp1) * flux_factor(1:kproma)
    ! net
    ...
    !
    ! diagnostics for cloud radiative forcing
    IF (  locfdiag ) THEN
      DO jk = 1, klevp1
    ...
    !IF ( raddiag ) THEN
    ! ! placeholder for detailed diagnostics for radiative transfer for usage in RFMIP 
    !ENDIF
  ...
  SUBROUTINE setup_radiation
    ...
    LOGICAL :: l_aero_kinne, l_aero_volc, l_aero_prop_ham, &
               l_aero_prop_crow, l_sp_setup
    !
    ...
    IF (p_parallel) THEN
      ...
      CALL p_bcast (ldiag_aeropt, p_io)
      ...
      CALL p_bcast (iaero_forcing, p_io)
      ...
      CALL p_bcast (solc_multiplier, p_io)
    ...
    IF (lrad) THEN
      ...
      l_aero_kinne     = .FALSE.
      l_aero_volc      = .FALSE.
      l_aero_prop_ham  = .FALSE.
      l_aero_prop_crow = .FALSE.
      l_sp_setup       = .FALSE.
      SELECT CASE (iaero)
      CASE(0)
      ...
      CASE(6)
        ...
        l_aero_kinne    = .TRUE.
        l_aero_volc     = .TRUE.
        l_aero_prop_ham = .TRUE.
      CASE(7)
        CALL message('','iaero= 7 --> Kinne climatology + Crowley volcanic aerosol')
        l_aero_kinne     = .TRUE.
        l_aero_prop_crow = .TRUE.
      CASE(8)
        CALL message('','iaero= 8 --> Kinne climatology + Simple Plumes + Stenchikov volcanic aerosol')
        l_aero_kinne = .TRUE.
        l_aero_volc  = .TRUE.
        l_sp_setup   = .TRUE.
      CASE(9)
        CALL message('','iaero= 9 --> Kinne climatology + Simple Plumes')
        l_aero_kinne = .TRUE.
        l_sp_setup   = .TRUE.
      CASE default
        WRITE (message_text, '(a,i2,a)') &
      ...
      END SELECT
      SELECT CASE (iaero_forcing)
      CASE(0)
        CALL message('','iaero_forcing= 0 --> no aerosol in radiation')
      CASE(1)
        CALL message('','iaero_forcing= 1 --> prognostic aerosol (sub model)')
      CASE(3)
        CALL message('','iaero_forcing= 3 --> Kinne climatology')
        l_aero_kinne = .TRUE.
      CASE(5)
        CALL message('','iaero_forcing= 5 --> Kinne climatology + Stenchikov volcanic aerosol')
        l_aero_kinne = .TRUE.
        l_aero_volc  = .TRUE.
      CASE(6)
        CALL message('','iaero_forcing= 6 --> Kinne climatology + Stenchikov volcanic aerosols + HAM volcanic aerosol')
        l_aero_kinne    = .TRUE.
        l_aero_volc     = .TRUE.
        l_aero_prop_ham = .TRUE.
      CASE(7)
        CALL message('','iaero_forcing= 7 --> Kinne climatology + Crowley volcanic aerosol')
        l_aero_kinne     = .TRUE.
        l_aero_prop_crow = .TRUE.
      CASE(8)
        CALL message('','iaero_forcing= 8 --> Kinne climatology + Simple Plumes + Stenchikov volcanic aerosol')
        l_aero_kinne = .TRUE.
        l_aero_volc  = .TRUE.
        l_sp_setup   = .TRUE.
      CASE(9)
        CALL message('','iaero_forcing= 9 --> Kinne climatology + Simple Plumes')
        l_aero_kinne = .TRUE.
        l_sp_setup   = .TRUE.
      CASE default
        WRITE (message_text, '(a,i2,a)') &
             'iaero_forcing=', iaero_forcing, ' in radctl namelist is not supported'
        CALL message('',message_text)
        CALL finish('setup_radiation','Run terminated iaero_forcing')
      END SELECT
      IF (l_aero_kinne)     CALL su_aero_kinne(nb_sw)
      IF (l_aero_volc)      CALL su_aero_volc(nb_sw)
      IF (l_aero_prop_ham)  CALL su_aero_prop_ham
      IF (l_aero_prop_crow) CALL su_aero_prop_crow
      IF (l_sp_setup)       CALL sp_setup
      IF (l_aero_prop_ham.OR.l_aero_prop_crow) CALL read_aero_volc_tables
      IF (l_aero_prop_ham.AND.l_aero_prop_crow) THEN
         CALL finish('setup_radiation','Both, Crowley and HAM aerosols can '//&
              'not be used at the same time for internal reasons')
      END IF
      !
      ! --- Check annual cycle
      ! 
      ...
      CASE (7) 
        CALL message('','isolrad = 7 --> CMIP6 1950 solar constant')
      CASE default 
      ...
      CALL message('',message_text)
      WRITE (message_text,'(a67,L3)') 'diagnostics of instantaneous aerosol optical properties: ',&
           ldiag_aeropt
      CALL message('',message_text)
      !
