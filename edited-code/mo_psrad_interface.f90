...
module mo_psrad_interface
  ...
  USE mo_radiation_parameters, ONLY: rad_perm, psctm, ssi_factor, ldiag_aeropt
  ...
  USE mo_simple_plumes,   ONLY: add_aop_sp
  ...
  USE mo_aeropt_stream,   ONLY: diagnose_aeropt_stream
...
CONTAINS
  ...
  SUBROUTINE psrad_interface( &
    ...
    INTEGER, INTENT(IN) :: iaero(2)       !< aerosol control
    ...
    ! -------------------------------------------------------------------------------------
    INTEGER  :: jk, jl, jp, jkb, jwl,     & !< loop indicies
    ...
    REAL(wp) ::                             &
         ...
         indirect_tau_sw_vr(kbdim,klev,nb_sw),  & !< dummy aerosol optical thickness for indirect effect case
         indirect_cg_sw_vr (kbdim,klev,nb_sw),  & !< dummy aerosol asymmetry factor for indirect effect case
         indirect_piz_sw_vr(kbdim,klev,nb_sw)     !< dummy aerosol single scattering albedo for indirect effect case
...
!IBM* ASSERT(NODEPS)
    ...
    CASE (8)
      ...
      DO jk = 1, klev
        DO jl = 1, kproma
          cdnc_vr(jl,jk) = cdnc_vr(jl,jk) * x_cdnc(jl)
        END DO
      END DO
...
! ================================================================================================================================
    CASE (10, 12) !  to calculate forcing of direct effect from anthropogenic aerosols (without affecting cloud optics)
                  !  12 = if some volcanic aerosols are present (if current state case is case 8 (historical)), then add volcanoes
      CALL set_aop_kinne( &
           & kproma           ,kbdim                 ,klev             ,&
           & krow             ,nbndlw                ,nb_sw            ,&
           & aer_tau_lw_vr    ,aer_tau_sw_vr         ,aer_piz_sw_vr    ,&
      ...
      CASE (:9)
          ...
          & krow             ,nb_sw                 ,aer_tau_sw_vr    ,&
          & aer_piz_sw_vr    ,aer_cg_sw_vr          ,x_cdnc           ,&
          & (iaero(2)+20))    !< ex: 21 for direct effect of plume 1 only  
      ...
      CASE (11:)
        ...
        & krow             ,nb_sw                 ,aer_tau_sw_vr    ,&
        & aer_piz_sw_vr    ,aer_cg_sw_vr          ,x_cdnc           ,&
        & (iaero(2)+20)) ! ex: 31 for direct and indirect effects of all plumes but indirect effect of plume 1
        ! indirect effect of all other plumes only in this condition  
        DO jk = 1, klev
          DO jl = 1, kproma
            cdnc_vr(jl,jk) = cdnc_vr(jl,jk) * x_cdnc(jl)
          END DO
      ...
      IF (iaero(1)==12) THEN ! If volcanoes
        CALL add_aop_volc( &
        & kproma           ,kbdim                 ,klev             ,&
        & krow             ,nbndlw                ,nb_sw            ,&
        & aer_tau_lw_vr    ,aer_tau_sw_vr         ,aer_piz_sw_vr    ,&
...
! ===========================================================================================================================
    CASE (11, 13) ! to calculate forcing from indirect effect of anthropogenic aerosol (without affecting aerosol properties)
                  ! 13 = if some volcanic aerosols are present (if current state case is case 8 (historical)), then add volcanoes
      CALL set_aop_kinne( &
            & kproma           ,kbdim                 ,klev             ,&
            & krow             ,nbndlw                ,nb_sw            ,&
            & aer_tau_lw_vr    ,aer_tau_sw_vr         ,aer_piz_sw_vr    ,&
      ...
      CASE (10)
        DO jk=1,klev
          DO jwl=1,nb_sw
              indirect_tau_sw_vr(1:kproma,jk,jwl) = aer_tau_sw_vr(1:kproma,jk,jwl)
              indirect_piz_sw_vr(1:kproma,jk,jwl) = aer_piz_sw_vr(1:kproma,jk,jwl)
              indirect_cg_sw_vr(1:kproma,jk,jwl) = aer_cg_sw_vr(1:kproma,jk,jwl)
          END DO
        END DO
        CALL add_aop_sp( &
              & kproma           ,kbdim                 ,klev             ,&
              & krow             ,nb_sw                 ,indirect_tau_sw_vr    ,&
              & indirect_piz_sw_vr    ,indirect_cg_sw_vr          ,x_cdnc      ,&
              & iaero(2))
      ...
      CASE (11:)
        CALL add_aop_sp( &
            ...
            & krow             ,nb_sw                 ,aer_tau_sw_vr    ,&
            & aer_piz_sw_vr    ,aer_cg_sw_vr          ,x_cdnc      ,&
            & (iaero(2)+40)) !< ex: 51 for direct and indirect effect of all plumes but direct of plume 1
      ...
      CASE (:9)
        ...
        CALL add_aop_sp( &
            ...
            & krow             ,nb_sw                 ,indirect_tau_sw_vr    ,&
            & indirect_piz_sw_vr    ,indirect_cg_sw_vr          ,x_cdnc      ,&
            & (iaero(2)+40)) !< ex: 41 for indirect effect of plume 1 only
      CASE DEFAULT
      END SELECT

      DO jk = 1, klev    
        DO jl = 1, kproma
          cdnc_vr(jl,jk) = cdnc_vr(jl,jk) * x_cdnc(jl) ! Case with contribution of anthro aerosols to cloud droplet number
        END DO
      END DO

      IF (iaero(1)==13) THEN ! IF volcanoes then add them
        CALL add_aop_volc( &
        & kproma           ,kbdim                 ,klev             ,&
        & krow             ,nbndlw                ,nb_sw            ,&
        & aer_tau_lw_vr    ,aer_tau_sw_vr         ,aer_piz_sw_vr    ,&
    ...
    IF (locosp) THEN
      DO jk=1,klev
        ...
        DO jl=1,kproma
          ...
          cosp_f3d(jl,jk,krow)   = cld_frc(jl,jk) !cms: Or cld_frc_vr? 
        END DO
      END DO
      IF ( Lisccp_sim ) THEN   
    ...
    IF (lanysubmodel)  CALL radiation_subm_1(                     &
         ...
         ktrac            ,iaero(1)            ,nbndlw       ,nb_sw ,&
         aer_tau_sw_vr    ,aer_piz_sw_vr    ,aer_cg_sw_vr        ,&
         aer_tau_lw_vr    , ppd_hl           ,pxtm1               )
    ! 3.6 Diagnose aerosol optical properies.
    ! IMPORTANT REMARK: If radiation is called repeatedly, the LAST
    ! call is with all aerosols used for model time integration
    ! Since the writing is at the end of a time step, no problem
    ! occurs
    IF (ldiag_aeropt) CALL diagnose_aeropt_stream(                &
         kproma           ,kbdim            ,klev                ,&
         krow             ,nbndlw           ,nb_sw               ,&
         aer_tau_sw_vr    ,aer_piz_sw_vr    ,aer_cg_sw_vr        ,&
         aer_tau_lw_vr    ,x_cdnc                                 )         
    !
    ...
    ! ------------------------------------------------------------------
    IF (lanysubmodel)  CALL radiation_subm_2(kproma, kbdim, krow, klev, ktrac, iaero(1), pxtm1)
